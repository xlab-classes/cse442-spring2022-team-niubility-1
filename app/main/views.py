    ...
    for i in range(len(date_list) - 1):
        ...
        for task in tasks:
            ...
            if task.priority == "Higher":
                last_name = '<div style="color: red">' + last_name + '</div>'
            elif task.priority == "Medium":
                last_name = '<div style="color: blue">' + last_name + '</div>'
            elif task.priority == "Lower":
                last_name = '<div style="color: green">' + last_name + '</div>'
            else:
                pass
...
def add_task():
    ...
    if request.method == 'POST':
        ...
        task = Task(name=name.replace("\n", ""), begin=begin_date_db, begin_time=begin_time, user_id=user.id, priority=priority)
        db.session.add(task)
